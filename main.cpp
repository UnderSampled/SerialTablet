	...
	while (1) {
		if (sizeof(szBuff)==SerialReadRaw(hSerial, szBuff, sizeof(szBuff)))
		{
			...
			if (State.values[WACOMFIELD_PROXIMITY].nValue)
			{
				...
				Input.mi.dx = ceil(State.values[WACOMFIELD_POSITION_X].nValue*2.15948333);
				Input.mi.dy = ceil(State.values[WACOMFIELD_POSITION_Y].nValue*3.45508224);
				::SendInput(1,&Input,sizeof(INPUT));
			}
			if (State.values[WACOMFIELD_BUTTONS].nValue & BIT(WACOMBUTTON_TOUCH))
			{
				::ZeroMemory(&Input,sizeof(INPUT));
